...
# Examples:
#   hack/build-docker.sh k8s-admission-webhook
#   BUILD_PLATFORMS=linux/amd64 hack/build-docker.sh k8s-admission-webhook
#   OUTPUT_TYPE=registry BUILD_PLATFORMS=linux/amd64,linux/arm64 hack/build-docker.sh k8s-admission-webhook
#   DOCKER_BUILD_ARGS="--build-arg https_proxy=${https_proxy}" hack/build-docker.sh k8s-admission-webhook"
#   SIGN_IMAGE="1"

REPO_ROOT=$(dirname "${BASH_SOURCE[0]}")/..
source "${REPO_ROOT}/util.sh"

REGISTRY=${REGISTRY:-"docker.io/neteric"}
...
function build_cross_image() {
  ...
  docker buildx build --output=type="${output_type}" \
          ...
          ${DOCKER_BUILD_ARGS} \
          --tag "${image_name}" \
          --file "${REPO_ROOT}/build/docker/buildx.Dockerfile" \
