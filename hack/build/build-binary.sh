...
# Examples:
#   hack/build.sh k8s-admission-webhook
#   BUILD_PLATFORMS=linux/amd64,linux/arm64 hack/build.sh k8s-admission-webhook
...
function build_binary_for_platform() {
  ...
  local gopkg="${GO_PACKAGE}/$(util::get_target_source $target)"
  set -x
